...
## 8) Quels sont les 10 héros avec le plus de soins par seconde?
...
## 10) Est ce que les healers avec le plus de soin par seconde sont ceux qui sont le plus joués ? 

Nous avions vu dans le graphique X (insérer le numéro de la question) les héros les plus joués. Il serait donc intéressant de vérifier si les healers qui prodiguent le plus de soins par seconde sont ceux qui sont aussi les plus plus utilisés. Pour ce faire, nous allons prendre les 2 datasets nécessaire: celui qui contient la feature PickRate et celui qui contient les stats de soins par seconde. Nous ne garderons que les 4 healers présents dans le dataset utilisé dans les 3 graphiques précédents. Après avoir trié, et fusionner, on se retrouvera avec un seul dataset. Il restera un seul problème: dans le jeu de données contenant la valeur Pick Rate, chaque héro est cité plusieurs fois, car son taux d'utilisation diffère selon l'élo dans lequel il est joué (bronze, argent, or etc..) J'ai donc choisi de faire la moyenne de ce taux, pôur avoir un aperçu global. Une fois la dataset fusionné, avec les bons héros et un seul Pick Rate à chaque fois, nous pourrons créer un nuage de points pour déceler une tendance ou non.
...
```
Le taux de sélection correspond à tous héros confondus, le teux de personnes qui prennent X personnage. Une fois encore, nous ne trouvons pas réellement de tendance. Nous pouvons tout de même noter que Ana et Mercy sont les héros qui font le plus de soins, et sont les deux les plus utilisés. Bien que Zenyatta heal plus que Lucio, ce dernier se retrouve quand même au dessus, sûrement parce que zen ne peut healer qu'une personne à la fois, tandis que lucio peut ditribuer du soin à plusieurs alliés à la fois. Pour Mercy, cela peut être dû au fait que cette dernière peut utiliser une de ces capacité pour réanimer des alliés.
...
ggplot(map_data, aes(x = long, y = lat, group = group, fill = TotalUSDPrize/1000)) +
  ...
  theme_minimal()
```
## x)Quelles sont les meilleures équipes d'Overwatch en termes de gains moyens par tournoi?
...
ggplot(top_teams, aes(x=reorder(TeamName, AverageEarningsPerTournament), y=AverageEarningsPerTournament/1000, fill=TeamName)) +
  ...
  geom_text(aes(label=round(AverageEarningsPerTournament/1000, 2)), vjust=-0.3, size=3.5, color="black") +
  theme_minimal() +
  ...
  labs(title="Top 20 des Équipes Overwatch par Gains Moyens par Tournoi", x="Équipe", y="Gains Moyens par Tournoi en milliers USD")
```
...
## x)Quelles nationalités sont les plus communes parmi les 100 joueurs d'Overwatch les mieux payés?
...
## x)Quelle est la distribution des gains totaux parmi les 100 meilleurs joueurs d'Overwatch, segmentée par rôle?
...
## x)Quels sont les gains moyens par rôle et par héros parmi les joueurs d'Overwatch?
