...
```{r}
heroes <- rename(heroes, Heal_per_sec = `Health per Sec`)

# Calculer les Heal per seconds pour chaque héros
heroes <- mutate(heroes, HPS = Heal_per_sec)
heroes <- filter(heroes, !grepl(" ", Heroes) & HPS > 1)
heroes <- arrange(heroes, desc(HPS))
top_10_heroes_hps <- head(heroes, 10)
