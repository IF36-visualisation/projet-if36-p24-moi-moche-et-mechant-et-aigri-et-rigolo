...
>>>>>>> bd2f623c430c56b9db4573337b2ca248dd8a626d
       x = "Héros",
       y = "Heal per second") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
<<<<<<< HEAD
## X) Est ce que les healers avec le plus de soin par seconde sont ceux qui sont le plus joués ? 
...
X) Il y a t-il un rôle moins joué chez les professionels, offrant potentiellement plus de chances de devenir profesionnel ?

On ne sait pas vraiment à quoi s'attendre, Timo qui a déjà joué au jeu pense que qu'il y aura peut-être un peu plus de joueurs Damage.

```{r}
role_proportions <- player_data %>%
  group_by(Role) %>%
  summarise(Count = n()) %>%
  mutate(Proportion = Count / sum(Count))

# Créer le graphique en barres empilées
ggplot(role_proportions, aes(x = "", y = Proportion, fill = Role)) +
  geom_bar(stat = "identity", width = 1) +
  coord_flip() +
  labs(title = "Répartition des rôles chez les professionels",
       x = NULL, y = NULL) +
  theme_minimal() +
  theme(axis.text.x = element_blank(),
        ...
        plot.title = element_text(size = 16, hjust = 0.5, color = "black")) +
  scale_fill_manual(values = c("#66c2a5", "#fc8d62", "#8da0cb")) +
  geom_text(aes(label = scales::percent(Proportion, accuracy = 0.1)), position = position_stack(vjust = 0.5), size = 5, color = "black")

```
C'est donc le cas il y a quand même 43% de joueurs Damage dans les joueurs professionels pour 28% égalité pour Support et Tank.
On peut donc penser qu'on aurait plus de chance de démarrer une carrière professionel en jouant Support ou Tank. 
Car on rappelle dans une partie d'Overwatch il y a obligatoirement 2 Support, 2 Tank et 2 Damage.

Si on estime que ces pourcentages reflètent également les joueurs en général, on peut conseiller au joueurs qui veulent éviter les files d'attentes de jouer Support ou Tank.
